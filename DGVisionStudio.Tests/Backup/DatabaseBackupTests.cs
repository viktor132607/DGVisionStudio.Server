...
public sealed class DatabaseBackupTests
{
    ...
    public async Task RestoreRequiresExplicitConfirmation()
    {
        ...
        Assert.IsType<BadRequestObjectResult>(await controller.RestoreAsync(null, null, default));
        Assert.IsType<BadRequestObjectResult>(await controller.RestoreAsync(null, "RESTORE", default));
    }
    ...
    public async Task FullRoundTripRemovesNewObjectsAndFailedRestoreRollsBack()
    {
        ...
        if (string.IsNullOrEmpty(configured)) return; // CI supplies an isolated PostgreSQL service.
