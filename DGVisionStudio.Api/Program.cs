...
using (var scope = app.Services.CreateScope())
{
	...
	await dbContext.Database.ExecuteSqlRawAsync("CREATE TABLE IF NOT EXISTS \"CalendarEvents\" (\"Id\" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, \"Title\" character varying(200) NOT NULL, \"EventType\" character varying(40) NULL, \"AssignedTo\" character varying(150) NULL, \"ClientName\" character varying(150) NULL, \"ClientPhone\" character varying(50) NULL, \"Location\" character varying(300) NULL, \"Description\" character varying(2000) NULL, \"Color\" character varying(20) NULL, \"StartAtUtc\" timestamp with time zone NOT NULL, \"EndAtUtc\" timestamp with time zone NOT NULL, \"CreatedAtUtc\" timestamp with time zone NOT NULL DEFAULT NOW(), \"UpdatedAtUtc\" timestamp with time zone NULL);");
	await dbContext.Database.ExecuteSqlRawAsync("CREATE INDEX IF NOT EXISTS \"IX_CalendarEvents_StartAtUtc\" ON \"CalendarEvents\" (\"StartAtUtc\");");
	await dbContext.Database.ExecuteSqlRawAsync("CREATE INDEX IF NOT EXISTS \"IX_CalendarEvents_EndAtUtc\" ON \"CalendarEvents\" (\"EndAtUtc\");");
	await dbContext.Database.ExecuteSqlRawAsync("CREATE INDEX IF NOT EXISTS \"IX_CalendarEvents_EventType\" ON \"CalendarEvents\" (\"EventType\");");
}
